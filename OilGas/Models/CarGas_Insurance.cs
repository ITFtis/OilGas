namespace OilGas.Models
{
    ...
    public partial class CarGas_Insurance
    {
        ...
        static object lockGetAllCarGas_Insurance = new object();
        public static IEnumerable<CarGas_Insurance> GetAllCarGas_Insurance(int cachetimer = 0)
        {
            if (cachetimer == 0) cachetimer = Constant.cacheTime;

            string key = "OilGas.Models.F22Holiday.GetAllCarGas_Insurance";
            var allHoliday = DouHelper.Misc.GetCache<IEnumerable<CarGas_Insurance>>(cachetimer, key);
            lock (lockGetAllCarGas_Insurance)
            {
                if (allHoliday == null)
                {
                    System.Data.Entity.DbContext OilGasModelContextExt = new OilGasModelContextExt();
                    Dou.Models.DB.IModelEntity<CarGas_Insurance> db = new Dou.Models.DB.ModelEntity<CarGas_Insurance>(OilGasModelContextExt);
        ...
        public static void ResetGetAllCarGas_Insurance()
        {
            string key = "OilGas.Models.F22Holiday.GetAllCarGas_Insurance";
