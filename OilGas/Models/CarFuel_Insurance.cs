namespace OilGas.Models
{
    ...
    public partial class CarFuel_Insurance
    {
        [Key]
       ...
       // [ColumnDef(Visible = false, VisibleEdit = true)]
        ...
        public static IEnumerable<CarFuel_Insurance> GetAllCarFuel_Insurance(int cachetimer = 0)
        {
            if (cachetimer == 0) cachetimer = Constant.cacheTime;

            string key = "OilGas.Models.F22Holiday.GetAllCarFuel_Insurance";
            var allHoliday = DouHelper.Misc.GetCache<IEnumerable<CarFuel_Insurance>>(cachetimer, key);
            lock (lockGetAllCarFuel_Insurance)
            {
                if (allHoliday == null)
                {
                    System.Data.Entity.DbContext OilGasModelContextExt = new OilGasModelContextExt();
                    Dou.Models.DB.IModelEntity<CarFuel_Insurance> db = new Dou.Models.DB.ModelEntity<CarFuel_Insurance>(OilGasModelContextExt);
        ...
        public static void ResetGetAllCarFuel_Insurance()
        {
            string key = "OilGas.Models.F22Holiday.GetAllCarFuel_Insurance";
