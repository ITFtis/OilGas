...
$(document).ready(function () {
    ...
    //列管現況
    $('#ddl_GSM_Situation').change(function () {
        var value = $(this).val();

        $('#div_GSM_Situation_Control').hide();
        $('#div_GSM_Situation_NotControl').hide();
        $('#ddl_GSM_Situation_Control').val('');
        $('#ddl_GSM_Situation_NotControl').val('');

        if (value == 1) {
            $('#div_GSM_Situation_Control').show();
        }
        else if (value == 2) {
            $('#div_GSM_Situation_NotControl').show();
        }
    })
    ...
    $('#btnExportBasicExcel').click(function () {
        ...
        $.ajax({
            url: app.siteRoot + 'CarFuel_BasicData_Log_List/ExportCarFuel_BasicData_List',
            datatype: "json",
            type: "POST",
            data: { objs: objs },
            success: function (data) {
    ...
    $('#btnRenewCache').click(function () {
        ...
        $.ajax({
            url: app.siteRoot + 'CarFuel_BasicData_Log_List/ResetExport',
            datatype: "json",
            type: "Get",
            success: function (data) {
                if (data.result) {
...
function iniUI() {
    ...
    //自助加油機數
    $('#self_gun').change(function () {
        if ($(this).prop("checked")) {
            $('#self_one_gun').prop('checked', true);
            $('#self_two_gun').prop('checked', true);
            $('#self_four_gun').prop('checked', true);
            $('#self_six_gun').prop('checked', true);
            $('#self_eight_gun').prop('checked', true);

            $('#self_one_gun').prop('disabled', false);
            $('#self_two_gun').prop('disabled', false);
            $('#self_four_gun').prop('disabled', false);
            $('#self_six_gun').prop('disabled', false);
            $('#self_eight_gun').prop('disabled', false);
        }
        else {
            $('#self_one_gun').prop('checked', false);
            $('#self_two_gun').prop('checked', false);
            $('#self_four_gun').prop('checked', false);
            $('#self_six_gun').prop('checked', false);
            $('#self_eight_gun').prop('checked', false);

            $('#self_one_gun').prop('disabled', true);
            $('#self_two_gun').prop('disabled', true);
            $('#self_four_gun').prop('disabled', true);
            $('#self_six_gun').prop('disabled', true);
            $('#self_eight_gun').prop('disabled', true);
        }
    })
}
