$(document).ready(function () {
    ...
    $('[data-fn="CaseType"]').change(function () {
        ...
        var $ddl = $('[data-fn="Business_theme"]');
        $ddl.empty();
        $ddl.trigger('change');

        helper.misc.showBusyIndicator();
        $.ajax({
            url: app.siteRoot + 'Audit_ReportMissing_statistics/GetCheckItemList',
            datatype: "json",
            type: "POST",
            ...
            success: function (data) {
                if (data.result) {
                    //set
                    $ddl.append('<option value title="所有集團">選擇集團</option>');
                    if (data.itemList.length != 0) {
                        $.each(data.itemList, function (index, v) {
                            $ddl.append('<option value=' + v.Value + '>' + v.Name + '</option>');
                        });
                    }
                } else {
                    alert("查核項目取得失敗：\n" + data.errorMessage);
