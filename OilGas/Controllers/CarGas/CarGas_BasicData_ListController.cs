...
{
    [Dou.Misc.Attr.MenuDef(Id = "CarGas_BasicData_List", Name = "現況資料-基本資料欄位清單", MenuPath = "汽車加氣站/B統計報表專區", Action = "Index", Index = 1, Func = Dou.Misc.Attr.FuncEnum.ALL, AllowAnonymous = false)]
    public class CarGas_BasicData_ListController : Controller
    {
        // GET: CarGas_BasicData_List
        ...
        public ActionResult ExportCarGas_BasicData_List(ReportData.ViewParams objs)
        {
            string error = "";
            string url = "";
            string folder = FileHelper.GetFileFolder(Code.TempUploadFile.汽車加氣站_B統計報表專區_現況資料_基本資料欄位清單);
            string fileTitle = "汽車加氣站_基本資料欄位清單";
            ...
            try
            {
                ...
                List<string> titles = new List<string>() { "汽車加氣站_現況資料，查詢條件:" };

                //1.取得資料
                System.Data.Entity.DbContext dbContext = new OilGasModelContextExt();
                var datas = OilGas.Rpt_CarGas_BasicData_List.GetAllDatas();
                var n = datas.Count();
                ...
                if (objs.columns != null)
                {
                    ...
                    Tank_place_type = objs.columns.Where(b => b.Id == "Tank_place_type").Count() > 0;
                }
                ...
                var output = datas.Select(a => new
                {
                    ...
                    汽車加氣站地址 = cbAddr ? a.汽車加氣站地址 : "",
                    汽車加氣站地號 = cbAddr ? a.汽車加氣站地號 : "",
                    ...
                    販售油品種類 = SaleSoilClass ? a.販售油品種類 : null,
                    儲槽位置 = Tank_place_type ? a.儲槽位置 : null,
                }).Distinct().ToList();
                ...
                foreach (var data in output)
                {
                    ...
                    if (objs.columns != null)
                    {
                        ...
                        if (cbAddr)
                        {
                            f.汽車加氣站地址 = data.汽車加氣站地址;
                            f.汽車加氣站地號 = data.汽車加氣站地號;
                        }
                        if (cbReport_date)
                        {
        ...
        public ActionResult ResetExport()
        {
            try
            {
                Rpt_CarGas_BasicData_List.ResetGetAllDatas();
                Rpt_CarGas_BasicData_List.GetAllDatas();
            }
            catch (Exception ex)
            {
