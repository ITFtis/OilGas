...
namespace OilGas.Controllers.CarGas
{
    ...
    public class CarGas_BasicData_Log_ListController : Controller
    {
        ...
        public ActionResult ExportCarGas_BasicData_Log_List(ReportData.ViewParams objs)
        {
            string error = "";
            string url = "";
            string folder = FileHelper.GetFileFolder(Code.TempUploadFile.汽車加氣站_B統計報表專區_變更歷程_基本資料欄位清單);
            string fileTitle = "汽車加氣站_變更歷程_基本資料欄位清單";
            ...
            try
            {
                ...
                List<string> titles = new List<string>() { "汽車加氣站_變更歷程，查詢條件:" };

                //1.取得資料
                System.Data.Entity.DbContext dbContext = new OilGasModelContextExt();
                var datas = OilGas.Rpt_CarGas_BasicData_Log_List.GetAllDatas();
                var n = datas.Count();
                ...
                var output = datas.Select(a => new
                {
                    ...
                    SoilServerData = cbSoilServerData ? a.SoilServerData : "",            //對應舊系統group by
        ...
        public ActionResult ResetExport()
        {
            try
            {
                Rpt_CarGas_BasicData_Log_List.ResetGetAllDatas();
                Rpt_CarGas_BasicData_Log_List.GetAllDatas();
            }
            catch (Exception ex)
            {
