...
namespace OilGas.Controllers.CarGas
{
    ...
    public class CarGas_ClosedController : AGenericModelController<vw_CarGas_Closed_show>
    {
        ...
        protected override Dou.Models.DB.IModelEntity<OilGas.Models.vw_CarGas_Closed_show> GetModelEntity()
        {
            return new Dou.Models.DB.ModelEntity<OilGas.Models.vw_CarGas_Closed_show>(new OilGasModelContextExt());
        }
        protected override IEnumerable<vw_CarGas_Closed_show> GetDataDBObject(IModelEntity<vw_CarGas_Closed_show> dbEntity, params KeyValueParams[] paras)
        {
            ...
            if (filter == null)
            {
                return new List<vw_CarGas_Closed_show>().AsQueryable();
            }

            var dbContext = new OilGasModelContextExt();
            Dou.Models.DB.IModelEntity<vw_CarGas_Closed> model = new Dou.Models.DB.ModelEntity<vw_CarGas_Closed>(dbContext);
            ...
            {
                _vwCGCs = new List<vw_CarGas_Closed_show>();
                return new List<vw_CarGas_Closed_show>().AsQueryable();
            }
            ...
            //將資料匯入畫面顯示用的Class
            var lsColumeSumShow = ColumeSum.Select(a => new vw_CarGas_Closed_show
            {
                CityName = a.CityName,
                cpc = a.cpc,
        ...
        /// <returns></returns>
        public ActionResult ExportCarGas_Closed(params KeyValueParams[] paras)
        {
            string error = "";
            string url = "";
            string folder = FileHelper.GetFileFolder(Code.TempUploadFile.汽車加氣站_B統計報表專區_各縣市暫停營業家數統計報表);
            string fileTitle = "汽車加氣站_各縣市暫停營業家數統計報表";
        ...
        protected string getStrHtml()
        {
            ...
            DataTable dt = StatisticReportFunc.ConvertToDataTable(_vwCGCs);
