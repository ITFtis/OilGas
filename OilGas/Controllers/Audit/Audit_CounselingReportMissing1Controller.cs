...
{
    [Dou.Misc.Attr.MenuDef(Id = "Audit_CounselingReportMissing1", Name = "各縣市出席率與查核缺失數比較圖", MenuPath = "查核輔導專區/G輔導講習專區", Action = "Index", Index = 4, Func = Dou.Misc.Attr.FuncEnum.None, AllowAnonymous = false)]
    public class Audit_CounselingReportMissing1Controller : AGenericModelController<Audit_CounselingReportMissing1>
    {
        static List<Audit_CounselingReportMissing1> _lsAuditCRM1 = new List<Audit_CounselingReportMissing1>();
        static List<ChartAuditCRM1> _lsChartAuditCRM1 = new List<ChartAuditCRM1>();
        ...
        public ActionResult GetChartData()
        {
            ...
            var lstquery = _lsChartAuditCRM1;
            return Json(lstquery, JsonRequestBehavior.AllowGet);
        }

        protected override Dou.Models.DB.IModelEntity<OilGas.Models.Audit_CounselingReportMissing1> GetModelEntity()
        {
            return new Dou.Models.DB.ModelEntity<OilGas.Models.Audit_CounselingReportMissing1>(new OilGasModelContextExt());
        }

        protected override IEnumerable<Audit_CounselingReportMissing1> GetDataDBObject(IModelEntity<Audit_CounselingReportMissing1> dbEntity, params KeyValueParams[] paras)
        {
            ...
            {
                return new List<Audit_CounselingReportMissing1>().AsQueryable();
            }
            ...
            mergedt.Columns.Add(new DataColumn() { ColumnName = "CityCode", DataType = typeof(String) });
            ...
            if (workYear1 != "")
            {
                ...
                foreach (DataRow item in tmpdt1.Rows)
                {
                    ...
                    dr["CityCode"] = item["CityCode"].ToString();
            ...
            _lsChartAuditCRM1 = StatisticReportFunc.ConvertToList<ChartAuditCRM1>(mergedt);
            _lsAuditCRM1 = StatisticReportFunc.ConvertToList<Audit_CounselingReportMissing1>(getListData(workYear1, workYear2, workYear3, workYear4));
        ...
        protected string getStrHtml()
        {
            ...
            DataTable dt = StatisticReportFunc.ConvertToDataTable(_lsAuditCRM1);
            ...
            string Title = string.Format(@"<tr>" +
                                       "  <td rowspan=\"2\" align=\"center\">縣市別</td>" +
        ...
        public ActionResult ExportAudit_CounselingReportMissing1()
        {
            string error = "";
            string url = "";
            string folder = FileHelper.GetFileFolder(Code.TempUploadFile.查核輔導專區_G輔導講習會專區_各縣市出席率與查核缺失數比較);
            string fileTitle = "各縣市出席率與查核缺失數比較";
        ...
        private DataTable getCounselingRate(string workYear)
        {
            string sql = "";
            sql += string.Format(@"
  SELECT  [CityName] as workItem
      ,[CityCode]
      ,[Rank]
      ,[GSLCode],
	  (select count(distinct s_CaseNo) from CounselingData where s_year='{0}' and s_CaseNo is not null and s_CityCode=a.CityCode) as MolecularCount,
	  (select sum(Total) from Gas_Total_TempV where CaseType='CarFuel_BasicData' and area=a.CityCode) as DenominatorCount,
      case when isnull((select sum(Total) from Gas_Total_TempV where CaseType='CarFuel_BasicData' and area=a.CityCode),0)=0 then 0 else convert(decimal(6, 2),isnull((select count(distinct s_CaseNo) from CounselingData where s_year='{0}' and s_CaseNo is not null and s_CityCode=a.CityCode),0) * 100.0/isnull((select sum(Total) from Gas_Total_TempV where CaseType='CarFuel_BasicData' and area=a.CityCode),1)) end as Average
  FROM [dbo].[CityCode] a
  ...
  SELECT  [CityName] as workItem
      ...
      ,[Rank]
      ,[GSLCode],
	  (select count(distinct CaseNo) from Check_Basic_View where convert(varchar,year(CheckDate))='{0}' and CaseNo is not null and AreaCode=a.CityCode) as MolecularCount,
	  (select sum(isnull(AllDoesmeet,0)) from Check_Basic_View where convert(varchar,year(CheckDate))='{0}' and CaseNo is not null and  AreaCode=a.CityCode) as DenominatorCount,
      case when isnull((select count(distinct CaseNo) from Check_Basic_View where convert(varchar,year(CheckDate))='{0}' and CaseNo is not null and  AreaCode=a.CityCode),0)=0 then 0 
        else convert(decimal(6, 2),isnull((select sum(isnull(AllDoesmeet,0)) from Check_Basic_View where convert(varchar,year(CheckDate))='{0}' and CaseNo is not null and  AreaCode=a.CityCode),0) * 1.0/isnull((select count(distinct CaseNo) from Check_Basic_View where convert(varchar,year(CheckDate))='{0}' and CaseNo is not null and AreaCode=a.CityCode),1)) end as Average
  FROM [dbo].[CityCode] a
  order by Rank", workYear);
        ...
        private DataTable getListData(string workYear1, string workYear2, string workYear3, string workYear4)
        {
	        ...
	        ,(select count(distinct s_CaseNo) from CounselingData where s_year='{0}' and s_CaseNo is not null and s_CityCode=a.CityCode) as AttendSCount1
	        ,(select sum(Total) from Gas_Total_TempV where CaseType='CarFuel_BasicData' and area=a.CityCode) as DenominatorCount1
            ,case when isnull((select sum(Total) from Gas_Total_TempV where CaseType='CarFuel_BasicData' and area=a.CityCode),0)=0 then 0 else convert(decimal(6, 2),isnull((select count(distinct s_CaseNo) from CounselingData where s_year='{0}' and s_CaseNo is not null and s_CityCode=a.CityCode),0) * 100.0/isnull((select sum(Total) from Gas_Total_TempV where CaseType='CarFuel_BasicData' and area=a.CityCode),1)) end as CounselingRate1
            ", workYear1);
            }
            else
            {
	        ...
	        ,(select count(distinct s_CaseNo) from CounselingData where s_year='{0}' and s_CaseNo is not null and s_CityCode=a.CityCode) as AttendSCount2
	        ,(select sum(Total) from Gas_Total_TempV where CaseType='CarFuel_BasicData' and area=a.CityCode) as DenominatorCount2
            ,case when isnull((select sum(Total) from Gas_Total_TempV where CaseType='CarFuel_BasicData' and area=a.CityCode),0)=0 then 0 else convert(decimal(6, 2),isnull((select count(distinct s_CaseNo) from CounselingData where s_year='{0}' and s_CaseNo is not null and s_CityCode=a.CityCode),0) * 100.0/isnull((select sum(Total) from Gas_Total_TempV where CaseType='CarFuel_BasicData' and area=a.CityCode),1)) end as CounselingRate2
            ", workYear2);
            }
            else
            {
            ...
            ,isnull((select count(distinct CaseNo) from Check_Basic_View where convert(varchar,year(CheckDate))='{0}' and CaseNo is not null and AreaCode=a.CityCode),0) as SumCheckCount1
	        ,isnull((select sum(isnull(AllDoesmeet,0)) from Check_Basic_View where convert(varchar,year(CheckDate))='{0}' and CaseNo is not null and  AreaCode=a.CityCode),0) as SumCheckError1
            ,case when isnull((select count(distinct CaseNo) from Check_Basic_View where convert(varchar,year(CheckDate))='{0}' and CaseNo is not null and  AreaCode=a.CityCode),0)=0 then 0 
                else convert(decimal(6, 2),isnull((select sum(isnull(AllDoesmeet,0)) from Check_Basic_View where convert(varchar,year(CheckDate))='{0}' and CaseNo is not null and  AreaCode=a.CityCode),0) * 1.0/isnull((select count(distinct CaseNo) from Check_Basic_View where convert(varchar,year(CheckDate))='{0}' and CaseNo is not null and AreaCode=a.CityCode),1)) end as Average1
            ", workYear3);
            }
            else
            {
            ...
            ,isnull((select count(distinct CaseNo) from Check_Basic_View where convert(varchar,year(CheckDate))='{0}' and CaseNo is not null and AreaCode=a.CityCode),0) as SumCheckCount2
	        ,isnull((select sum(isnull(AllDoesmeet,0)) from Check_Basic_View where convert(varchar,year(CheckDate))='{0}' and CaseNo is not null and  AreaCode=a.CityCode),0) as SumCheckError2
            ,case when isnull((select count(distinct CaseNo) from Check_Basic_View where convert(varchar,year(CheckDate))='{0}' and CaseNo is not null and  AreaCode=a.CityCode),0)=0 then 0 
                else convert(decimal(6, 2),isnull((select sum(isnull(AllDoesmeet,0)) from Check_Basic_View where convert(varchar,year(CheckDate))='{0}' and CaseNo is not null and  AreaCode=a.CityCode),0) * 1.0/isnull((select count(distinct CaseNo) from Check_Basic_View where convert(varchar,year(CheckDate))='{0}' and CaseNo is not null and AreaCode=a.CityCode),1)) end as Average2
            ", workYear4);
            }
            else
            {
