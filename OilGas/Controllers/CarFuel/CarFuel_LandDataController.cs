...
{
    [Dou.Misc.Attr.MenuDef(Id = "CarFuel_LandData", Name = "已開業汽、機車加油站分析統計報表", MenuPath = "加油站/A統計報表專區", Action = "Index", Index = 6, Func = Dou.Misc.Attr.FuncEnum.None, AllowAnonymous = false)]   
    public class CarFuel_LandDataController : AGenericModelController<CarFuel_LandData>
    {
        // GET: CarFuel_LandData
        public ActionResult Index()
        {
            return View();
        }

        protected override Dou.Models.DB.IModelEntity<OilGas.Models.CarFuel_LandData> GetModelEntity()
        {
            return new Dou.Models.DB.ModelEntity<OilGas.Models.CarFuel_LandData>(new OilGasModelContextExt());
        }
        ...
        public ActionResult ExportCarFuelLandDataView(params KeyValueParams[] paras)
        {
            string error = "";
            string url = "";
            string folder = FileHelper.GetFileFolder(Code.TempUploadFile.加油站_A統計報表專區_已開業汽機車加油站分析統計報表);
            string fileTitle = "加油站_統計報表專區_已開業汽機車加油站分析統計報表";         
            List<string> titles = new List<string>() { "加油站_已開業汽機車加油站分析統計報表，查詢條件:" };
        ...
        protected string getExcel()
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                //報表名稱
                string ReportName = "已開業汽、機車加油站分析統計報表";
                //加總
