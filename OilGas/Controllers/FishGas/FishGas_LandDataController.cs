...
{
    [Dou.Misc.Attr.MenuDef(Id = "FishGas_LandData", Name = "已開業漁船加油站分析統計報表", MenuPath = "漁船加油站/C統計報表專區", Action = "Index", Index = 7, Func = Dou.Misc.Attr.FuncEnum.None, AllowAnonymous = false)]
    public class FishGas_LandDataController : AGenericModelController<FishGas_LandData>
    {
        // GET: FishGas_LandData
        public ActionResult Index()
        {
            return View();
        }

        protected override Dou.Models.DB.IModelEntity<OilGas.Models.FishGas_LandData> GetModelEntity()
        {
            return new Dou.Models.DB.ModelEntity<OilGas.Models.FishGas_LandData>(new OilGasModelContextExt());
        }
        ...
        public ActionResult ExportFishGasLandDataView(params KeyValueParams[] paras)
        {
            string error = "";
            string url = "";
            string folder = FileHelper.GetFileFolder(Code.TempUploadFile.漁船加油站_C統計報表專區_已開業漁船加油站分析統計報表);
            string fileTitle = "漁船加油站_統計報表專區_已開業漁船加油站分析統計報表";
            List<string> titles = new List<string>() { "漁船加油站_已開業漁船加油站分析統計報表，查詢條件:" };
        ...
        protected string getExcel()
        {
            ...
            if (dt != null && dt.Rows.Count > 0)
            {
                ...
                //報表名稱
                string ReportName = "已開業漁船加油站分析統計報表";
                //加總
