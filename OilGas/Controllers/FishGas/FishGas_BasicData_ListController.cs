...
{
    [Dou.Misc.Attr.MenuDef(Id = "FishGas_BasicData_List", Name = "現況資料-基本資料欄位清單", MenuPath = "漁船加油站/C統計報表專區", Action = "Index", Index = 1, Func = Dou.Misc.Attr.FuncEnum.ALL, AllowAnonymous = false)]
    public class FishGas_BasicData_ListController : Controller
    {
        // GET: FishGas_BasicData_List
        ...
        public ActionResult ExportFishGas_BasicData_List(ReportData.ViewParams objs)
        {
            string error = "";
            string url = "";
            string folder = FileHelper.GetFileFolder(Code.TempUploadFile.漁船加油站_C統計報表專區_現況資料_基本資料欄位清單);
            string fileTitle = "漁船加油站_基本資料欄位清單";
            ...
            try
            {
                ...
                List<string> titles = new List<string>() { "漁船加油站_現況資料，查詢條件:" };

                //1.取得資料
                System.Data.Entity.DbContext dbContext = new OilGasModelContextExt();
                var datas = OilGas.Rpt_FishGas_BasicData_List.GetAllDatas();
                var n = datas.Count();
