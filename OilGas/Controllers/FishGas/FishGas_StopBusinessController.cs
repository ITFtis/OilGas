...
{
    [Dou.Misc.Attr.MenuDef(Id = "FishGas_StopBusiness", Name = "各縣市新設、停業、復業與歇業之漁船加油站家數統計報表", MenuPath = "漁船加油站/C統計報表專區", Action = "Index", Index = 9, Func = Dou.Misc.Attr.FuncEnum.ALL, AllowAnonymous = false)]
    public class FishGas_StopBusinessController : AGenericModelController<FishGas_StopBusiness>
    {
        public static List<FishGas_StopBusiness> _vwFGSB = new List<FishGas_StopBusiness>();
        static string _Year_Start = "";
        static DataTable _DataTable = new DataTable();

        // GET: FishGas_StopBusiness
        ...
        protected override IEnumerable<FishGas_StopBusiness> GetDataDBObject(IModelEntity<FishGas_StopBusiness> dbEntity, params KeyValueParams[] paras)
        {
            ...
            if (string.IsNullOrEmpty(_Year_Start))
            {
                return new List<FishGas_StopBusiness>().AsQueryable();
            }

            var tmpdt = getData(_Year_Start);

            List<FishGas_StopBusiness> FishGas_StopBusiness = new List<FishGas_StopBusiness>();
            FishGas_StopBusiness = (from DataRow dr in tmpdt.Rows
                                   select new FishGas_StopBusiness()
                                   {
            ...
            _vwFGSB = FishGas_StopBusiness.Where(x => pCitys.Contains(x.CityCode)).ToList();
            return _vwFGSB;
        }
        protected override Dou.Models.DB.IModelEntity<OilGas.Models.FishGas_StopBusiness> GetModelEntity()
        {
            return new Dou.Models.DB.ModelEntity<OilGas.Models.FishGas_StopBusiness>(new OilGasModelContextExt());
        }
        ...
        private DataTable getData(string Year_Start)
        {
	        ...
	        ,[Rank]
            ,GSLCode
	        ,(Select count(distinct CaseNo) From FishGas_Dispatch p Left Join CityCode cc with(nolock) On GSLCode LIKE '%' + SUBSTRING(p.CaseNo, 5, 2) + '%' where DispatchClass in ('17') and cc.CityCode=a.CityCode and Year(Dispatch_date)={0} ) as AddBusiness
	        ,(Select count(distinct CaseNo) From FishGas_Dispatch p Left Join CityCode cc with(nolock) On GSLCode LIKE '%' + SUBSTRING(p.CaseNo, 5, 2) + '%' where DispatchClass in ('44','46') and cc.CityCode=a.CityCode and Year(Dispatch_date)={0} ) as StopBusiness
	        ,(Select count(distinct CaseNo) From FishGas_Dispatch p Left Join CityCode cc with(nolock) On GSLCode LIKE '%' + SUBSTRING(p.CaseNo, 5, 2) + '%' where DispatchClass in ('54') and cc.CityCode=a.CityCode and Year(Dispatch_date)={0} ) as ReBusiness
	        ,(Select count(distinct CaseNo) From FishGas_Dispatch p Left Join CityCode cc with(nolock) On GSLCode LIKE '%' + SUBSTRING(p.CaseNo, 5, 2) + '%' where DispatchClass in ('18','60','61','62','66','68') and cc.CityCode=a.CityCode and Year(Dispatch_date)={0} 
             and CaseNo not in (select CaseNo From FishGas_Dispatch p Left Join CityCode cc with(nolock) On GSLCode LIKE '%' + SUBSTRING(p.CaseNo, 5, 2) + '%' where DispatchClass in ('18','60','61','62','66','68') and cc.CityCode=a.CityCode and Year(Dispatch_date) < {0}) ) as EndBusiness
	        ,0 as Business
             From CityCode a
             order by Rank
            ", workYear1, workYear2);
        ...
        /// <returns></returns>
        public ActionResult ExportFishGas_StopBusiness(params KeyValueParams[] paras)
        {
            string error = "";
            string url = "";
            string folder = FileHelper.GetFileFolder(Code.TempUploadFile.漁船加油站_C統計報表專區_各縣市停歇業與新設加油站家數統計報表);
            string fileTitle = "漁船加油站_各縣市停歇業與新設加油站家數統計報表";
        ...
        protected string getStrHtml()
        {
            ...
            DataTable dt = StatisticReportFunc.ConvertToDataTable(_vwFGSB);
        ...
        private DataTable getDetailData(string workCity, string workYear, string workType)
        {
            ...
            #region<決定撈取的欄位>
            dicCol.Add("CaseNo", "漁船加油站編號");
            dicCol.Add("Gas_Name", "漁船加油站名稱");
            ...
            if (workType != "Business")
            {
                if (workType == "AddBusiness")
                {
                    ...
                    and p.id not in (select p.id From FishGas_Dispatch where id < p.id and CaseNo =t.CaseNo and DispatchClass in ('17') and Year(Dispatch_date) = {1})
                ", strWhr1, workYear);
                }
                else if (workType == "StopBusiness")
                {
                    ...
                    and p.id not in (select p.id From FishGas_Dispatch where id < p.id and CaseNo =t.CaseNo and DispatchClass in ('44','46') and Year(Dispatch_date) = {1})
                ", strWhr1, workYear);
                ...
                else if (workType == "ReBusiness")
                {
                    ...
                    and p.id not in (select p.id From FishGas_Dispatch where id < p.id and CaseNo =t.CaseNo and DispatchClass in ('54') and Year(Dispatch_date) = {1})
                ", strWhr1, workYear);
                }
                else if (workType == "EndBusiness")
                {
                    ...
                    and t.CaseNo not in (select CaseNo From FishGas_Dispatch where DispatchClass in ('18','60','61','62','66','68') and Year(Dispatch_date) < {1})
                    and p.id not in (select p.id From FishGas_Dispatch where id < p.id and CaseNo =t.CaseNo and DispatchClass in ('18','60','61','62','66','68') and Year(Dispatch_date) = {1})
                ", strWhr1, workYear);
                }
                strSQL += string.Format(@"select 
	                    ...
	                    ,cc.CityName
            from FishGas_Dispatch p 
            Left Join DispatchClassCode c with(nolock) On p.DispatchClass = c.Value
	        Left Join FishGas_BasicData t with(nolock) On p.CaseNo = t.CaseNo
        ...
        public JsonResult GetDetailTableData(workParas paras)
        {
            ...
            var lsCFWBD = StatisticReportFunc.ConvertToList<FishGas_WorkBusinessDesc>(_DataTable);
        ...
        /// <param name="paras"></param>
        /// <returns></returns>
        public ActionResult ExportFishGas_workBusinessDesc(workParas paras)
        {
            string error = "";
            string url = "";
            string folder = FileHelper.GetFileFolder(Code.TempUploadFile.漁船加油站_C統計報表專區_各縣市新設_停業與終止營運自用加儲油站家數統計報表);
            string fileTitle = "漁船加油站_各縣市新設_停業與終止營運自用加儲油站家數統計報表";
