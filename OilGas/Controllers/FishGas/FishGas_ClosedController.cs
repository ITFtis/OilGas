...
{
    [Dou.Misc.Attr.MenuDef(Id = "FishGas_Closed", Name = "各縣市暫停營業家數統計報表", MenuPath = "漁船加油站/C統計報表專區", Action = "Index", Index = 8, Func = Dou.Misc.Attr.FuncEnum.ALL, AllowAnonymous = false)]
    public class FishGas_ClosedController : AGenericModelController<vw_FishGas_Closed_show>
    {
        public static List<vw_FishGas_Closed_show> _vwFGCs = new List<vw_FishGas_Closed_show>();
        ...
        protected override Dou.Models.DB.IModelEntity<OilGas.Models.vw_FishGas_Closed_show> GetModelEntity()
        {
            return new Dou.Models.DB.ModelEntity<OilGas.Models.vw_FishGas_Closed_show>(new OilGasModelContextExt());
        }
        protected override IEnumerable<vw_FishGas_Closed_show> GetDataDBObject(IModelEntity<vw_FishGas_Closed_show> dbEntity, params KeyValueParams[] paras)
        {
            ...
            if (filter == null)
            {
                return new List<vw_FishGas_Closed_show>().AsQueryable();
            }

            var dbContext = new OilGasModelContextExt();
            Dou.Models.DB.IModelEntity<vw_FishGas_Closed> model = new Dou.Models.DB.ModelEntity<vw_FishGas_Closed>(dbContext);
            ...
            {
                _vwFGCs = new List<vw_FishGas_Closed_show>();
                return new List<vw_FishGas_Closed_show>().AsQueryable();
            }
            ...
            //將資料匯入畫面顯示用的Class
            var lsColumeSumShow = ColumeSum.Select(a => new vw_FishGas_Closed_show
            {
                CityName = a.CityName,
                cpc = a.cpc,
        ...
        /// <returns></returns>
        public ActionResult ExportFishGas_Closed(params KeyValueParams[] paras)
        {
            string error = "";
            string url = "";
            string folder = FileHelper.GetFileFolder(Code.TempUploadFile.漁船加油站_C統計報表專區_各縣市暫停營業家數統計報表);
            string fileTitle = "漁船加油站_各縣市暫停營業家數統計報表";
        ...
        protected string getStrHtml()
        {
            ...
            DataTable dt = StatisticReportFunc.ConvertToDataTable(_vwFGCs);
