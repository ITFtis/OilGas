...
{
    [Dou.Misc.Attr.MenuDef(Id = "PortGas_BasicDataLog", Name = "變更歷程資料匯出", MenuPath = "航港自用加儲油/E統計報表專區", Action = "Index", Index = 2, Func = Dou.Misc.Attr.FuncEnum.ALL, AllowAnonymous = false)]
    public class PortGas_BasicDataLogController : Controller
    {
        // GET: PortGas_BasicDataLog
        ...
        public ActionResult ExportPortGas_BasicDataLog(ReportData.ViewParams objs)
        {
            string error = "";
            string url = "";
            string folder = FileHelper.GetFileFolder(Code.TempUploadFile.航港自用加儲油_E統計報表專區_變更歷程);
            string fileTitle = "航港自用加儲油_變更歷程";
            ...
            try
            {
                ...
                List<string> titles = new List<string>() { "航港加儲油_變更歷程，查詢條件:" };

                //1.取得資料
                System.Data.Entity.DbContext dbContext = new OilGasModelContextExt();
                var datas = OilGas.Rpt_PortGas_BasicDataLog.GetAllDatas();
                var n = datas.Count();
        ...
        public ActionResult ResetExport()
        {
            try
            {
                Rpt_PortGas_BasicDataLog.ResetGetAllDatas();
                Rpt_PortGas_BasicDataLog.GetAllDatas();
            }
            catch (Exception ex)
            {
